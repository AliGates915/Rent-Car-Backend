...
export const getBookingStatisticsByDateRange = async (req, res) => {
  try {
    ...
    console.error('Statistics by date range error:', error);
    res.status(500).json({ error: error.message });
  }
};
...
export const getVehicleBookingHistory = async (req, res) => {
  try {
    ...
    console.error('Vehicle booking history error:', error);
    res.status(500).json({ error: error.message });
  }
};
...
export const getBookingTimeline = async (req, res) => {
  try {
    ...
    let statusLogs = [];
    try {
      const [logs] = await pool.query(statusLogsSql, [id]);
      statusLogs = logs;
    } catch (err) {
      // Table might not exist, ignore error
      console.log('Status logs table not found or error:', err.message);
    }
    ...
    console.error('Booking timeline error:', error);
    res.status(500).json({ error: error.message });
  }
};
...
export const exportBookingHistory = async (req, res) => {
  try {
    ...
    console.error('Export booking history error:', error);
    res.status(500).json({ error: error.message });
  }
};