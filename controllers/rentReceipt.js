...
export const createRentReceipt = async (req, res, next) => {
  try {
    ...
    // Save the new rent receipt to the database
...
export const deleteRentReceipt = async (req, res, next) => {
    try {
      const deletedRentReceipt = await RentReceipt.findByIdAndDelete(req.params.id);
      if (!deletedRentReceipt) {
        return res.status(404).json({ message: "Customer not found" });
      }
      res.status(200).json({ message: "Customer deleted successfully", deletedRentReceipt });
    } catch (error) {
